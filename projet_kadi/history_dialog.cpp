...
HistoryDialog::HistoryDialog(QSqlDatabase &db, QWidget *parent) 
    : QDialog(parent), m_db(db)
{
    ...
    connect(btnRestaurer, &QPushButton::clicked, this, &HistoryDialog::on_pushButton_restaurer_clicked);
    connect(btnSuppr, &QPushButton::clicked, this, &HistoryDialog::on_pushButton_supprimer_def_clicked);
...
int HistoryDialog::idSelectionne() const
{
...
void HistoryDialog::on_pushButton_restaurer_clicked()
{
    ...
    if (q.exec()) {
        QSqlQuery qDel(m_db);
        qDel.prepare("DELETE FROM navire_archive WHERE id_navire = :id");
...
void HistoryDialog::on_pushButton_supprimer_def_clicked()
{
    int id = idSelectionne();
    if (id < 0) return;

    if (QMessageBox::question(this, tr("Confirmation"), tr("Supprimer définitivement ce navire ? Cette action est irréversible.")) == QMessageBox::Yes) {
        QSqlQuery q(m_db);
        q.prepare("DELETE FROM navire_archive WHERE id_navire = :id");
        q.bindValue(":id", id);
