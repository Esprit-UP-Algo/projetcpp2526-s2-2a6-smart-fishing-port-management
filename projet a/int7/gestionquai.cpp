    ...
    : QWidget(parent)
{
    // ── Professional theme ────────────────────────────────────────────────────
    setStyleSheet(R"(
        QWidget { background-color:#f0f4f8; font-family:'Segoe UI',Arial,sans-serif;
                  font-size:13px; color:#0f172a; }
        QWidget#menuQuai { background:#0a1628; border-radius:10px; padding:8px; }
        QWidget#menuQuai QPushButton {
            background:#1d4ed8; color:white; border:none; border-radius:7px;
            padding:8px 12px; font-weight:700; text-align:left; min-height:36px; }
        QWidget#menuQuai QPushButton:hover  { background:#2563eb; }
        QWidget#menuQuai QPushButton:pressed{ background:#1e40af; }
        QWidget#pageFrame { background:#f0f4f8; border-radius:10px; }
        QWidget#contentFrame { background:#ffffff; border-radius:10px;
                               border:1px solid #e2e8f0; padding:16px; }
        QLabel#pageTitle { font-size:18px; font-weight:800; color:#0a1628; margin-bottom:8px; }
        QLineEdit, QSpinBox, QDoubleSpinBox, QComboBox, QDateEdit {
            background:#ffffff; border:1px solid #cbd5e1; border-radius:6px;
            padding:5px 8px; min-height:32px; color:#0f172a; }
        QLineEdit:focus, QSpinBox:focus, QDoubleSpinBox:focus,
        QComboBox:focus, QDateEdit:focus { border:2px solid #1d4ed8; }
        QPushButton { background:#1d4ed8; color:white; border:none; border-radius:7px;
        ...
        QTableWidget { background:#ffffff; gridline-color:#e2e8f0; border:1px solid #e2e8f0;
                       border-radius:8px; selection-background-color:#dbeafe; selection-color:#1e3a8a; }
        ...
        QHeaderView::section { background:#0a1628; color:white; padding:8px 6px;
        ...
        QScrollBar::handle:vertical { background:#94a3b8; border-radius:4px; min-height:30px; }
    )");
...
void PageQuai::ajouterQuai()
{
    ...
    if (!quaiDb.ajout()) {
        QSqlQuery diagQuery;
        diagQuery.exec("SELECT 1 FROM DUAL"); // force last error refresh
        QString errMsg = QSqlDatabase::database().lastError().text();
        // Try a direct query to get the real error
        QSqlQuery testQ;
        testQ.prepare(QString("INSERT INTO quai (id_quai, nom, places_total, navires_presents, adresse, etat, tirant_eau_max, equipements) "
                              "VALUES (:id,:nom,:places,:navires,:adresse,:etat,:tirant,:equip)"));
        testQ.bindValue(":id", quaiDb.id_quai);
        testQ.bindValue(":nom", quaiDb.nom);
        testQ.bindValue(":places", quaiDb.places_total);
        testQ.bindValue(":navires", quaiDb.navires_presents);
        testQ.bindValue(":adresse", quaiDb.adresse);
        testQ.bindValue(":etat", quaiDb.etat);
        testQ.bindValue(":tirant", quaiDb.tirant_eau_max);
        testQ.bindValue(":equip", quaiDb.equipements);
        testQ.exec();
        QString detail = testQ.lastError().text();
        QMessageBox::warning(this, "Erreur SQL", QString("Impossible d'ajouter le quai.\n\nDétail: %1").arg(detail.isEmpty() ? errMsg : detail));
        return;
    }
