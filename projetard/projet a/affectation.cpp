...
void AffectationWidget::refreshQuais()
{
    ...
    while (query.next()) {
        ...
        QSqlQuery cnt("SELECT COUNT(*) FROM affectations WHERE quai_id = :id AND statut = 'Confirmee'");
...
void AffectationWidget::lancerAnalyse()
{
    ...
    if (equipReq.isEmpty()) {
        ...
        for (int i = 1; i < equipReq.size(); ++i) {
            ...
            for (const QuaiDB &courant : quaisCompatibles) {
                for (const QuaiDB &suivant : suivants) {
    ...
    dernierResultat.clear();
    for (const QuaiDB &quai : quaisCompatibles) {
...
void AffectationWidget::afficherResultats()
{
    ...
    for (const ResultatScoring &resultat : dernierResultat) {
...
void AffectationWidget::confirmerAffectation()
{
    ...
    insertAffect.prepare("INSERT INTO affectations (navire_nom, tirant_navire, priorite, quai_id, date_arrivee, duree_estimee, score_final, statut) "
