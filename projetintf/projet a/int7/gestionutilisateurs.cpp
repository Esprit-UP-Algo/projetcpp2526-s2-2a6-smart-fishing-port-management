...
GestionUtilisateurs::GestionUtilisateurs(QWidget *parent) : QWidget(parent)
{
    ...
    QDir().mkpath(dataDir);
    m_jsonPath = QDir(dataDir).filePath("employee.json");
...
void GestionUtilisateurs::configurerInterface()
{
    ...
    mainFrame->setStyleSheet("background:#f0f4f8; border-radius:10px; margin:8px;");
...
{
    // No-op: saves are done immediately via SQL in ajouter/modifier/supprimer
}
    ...
    for (const Employe &e : listeEmployes) {
            ...
            for (int i = 0; i < listeEmployes.size(); i++) {
                if (listeEmployes[i].id == id) {
                    ...
                    if (dialog.exec() == QDialog::Accepted) {
                        ...
                        q.bindValue(":nom", listeEmployes[i].nom);
                        q.bindValue(":poste", listeEmployes[i].poste);
                        q.bindValue(":email", listeEmployes[i].email);
                        q.bindValue(":tel", listeEmployes[i].telephone);
                        q.bindValue(":sal", listeEmployes[i].salaire);
                        q.bindValue(":id", id);
        ...
        connect(btnDelete, &QPushButton::clicked, this, [this, row]() {
            ...
            if (QMessageBox::question(this, "Confirmation",
                ...
                if (!q.exec()) QMessageBox::warning(this, "Erreur SQL", q.lastError().text());
                for (int i = 0; i < listeEmployes.size(); i++) {
                    if (listeEmployes[i].id == id) { listeEmployes.removeAt(i); break; }
                }
                actualiserTable();
                mettreAJourStatistiques();
                mettreAJourStatistiquesSalaires();
...
void GestionUtilisateurs::ajouterEmploye()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        QSqlQuery q;
        q.prepare("INSERT INTO employee (id, nom, poste, email, telephone, salaire) VALUES (:id,:nom,:poste,:email,:tel,:sal)");
        q.bindValue(":id", e.id);
        q.bindValue(":nom", e.nom);
        q.bindValue(":poste", e.poste);
        q.bindValue(":email", e.email);
        q.bindValue(":tel", e.telephone);
        q.bindValue(":sal", e.salaire);
        if (!q.exec()) {
...
void GestionUtilisateurs::loadFromDb()
{
    listeEmployes.clear();
    QSqlQuery q;
    if (!q.exec("SELECT id, nom, poste, email, telephone, salaire FROM employee")) {
...
{
    setStyleSheet(R"(
        QWidget { background-color:#f0f4f8; font-family:'Segoe UI',Arial,sans-serif; font-size:13px; color:#0f172a; }
        QFrame#pageFrame { background:#f0f4f8; border-radius:10px; }
        QGroupBox { background:#ffffff; border:1px solid #e2e8f0; border-radius:10px;
                    margin-top:14px; padding:12px; font-weight:700; color:#0a1628; }
        ...
        QLineEdit:focus, QSpinBox:focus, QDoubleSpinBox:focus, QComboBox:focus { border:2px solid #1d4ed8; }
        QPushButton { background:#1d4ed8; color:white; border:none; border-radius:7px;
