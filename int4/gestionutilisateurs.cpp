    ...
    : QDialog(parent), isEditMode(isEditMode)
{
    ...
    titre->setStyleSheet("font-size: 22px; font-weight: bold; color: #1e3a8a; margin-bottom: 10px;");
    mainLayout->addWidget(titre);

    QFrame *formFrame = new QFrame;
    formFrame->setStyleSheet("background-color: #f8fafc; border-radius: 10px; padding: 25px; border: 1px solid #e2e8f0;");
    ...
    lineId->setMinimumHeight(40);
    lineId->setStyleSheet("background-color: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");

    lineNom = new QLineEdit;
    lineNom->setPlaceholderText("Nom complet");
    lineNom->setMinimumHeight(40);
    lineNom->setStyleSheet("border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");

    comboPoste = new QComboBox;
    comboPoste->addItems({"Pêcheur", "Matelot", "Docker", "Chef de quai", "Superviseur"});
    comboPoste->setMinimumHeight(40);
    comboPoste->setStyleSheet("border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");

    lineEmail = new QLineEdit;
    lineEmail->setPlaceholderText("email@example.com");
    lineEmail->setMinimumHeight(40);
    lineEmail->setStyleSheet("border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");

    lineTelephone = new QLineEdit;
    lineTelephone->setPlaceholderText("0600000000");
    lineTelephone->setMinimumHeight(40);
    lineTelephone->setStyleSheet("border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");

    spinSalaire = new QDoubleSpinBox;
    spinSalaire->setRange(0, 1000000);
    spinSalaire->setValue(35000);
    spinSalaire->setSuffix(" TND");
    spinSalaire->setMinimumHeight(40);
    spinSalaire->setStyleSheet("border: 1px solid #cbd5e1; border-radius: 6px; padding: 8px;");
    ...
    btnSave->setStyleSheet("background-color: #3b82f6; color: white; font-weight: bold; font-size: 15px; border-radius: 8px; padding: 10px;");
    btnCancel->setStyleSheet("background-color: #ef4444; color: white; font-weight: bold; font-size: 15px; border-radius: 8px; padding: 10px;");
...
void GestionUtilisateurs::configurerTableauBord()
{
    ...
    titre->setStyleSheet("font-size: 26px; font-weight: bold; color: #1e3a8a; margin-bottom: 10px;");

    QFrame *frameStats = new QFrame;
    frameStats->setStyleSheet("background-color: white; border-radius: 12px; border: 1px solid #e2e8f0; padding: 20px;");

    QVBoxLayout *statsInnerLayout = new QVBoxLayout(frameStats);

    QLabel *titreStats = new QLabel("📊 Répartition par poste");
    titreStats->setStyleSheet("font-size: 18px; font-weight: bold; color: #1e293b; margin-bottom: 15px;");

    QHBoxLayout *postesLayout = new QHBoxLayout;
    postesLayout->setSpacing(20);
    ...
    QString statStyle = "font-size: 15px; font-weight: bold; color: #1e3a8a; padding: 12px; background: #f1f5f9; border-radius: 10px; border-left: 5px solid #3b82f6;";
    ...
    frameSalaires->setStyleSheet("background-color: white; border-radius: 12px; border: 1px solid #e2e8f0; padding: 20px; margin-top: 5px;");

    QVBoxLayout *salairesInnerLayout = new QVBoxLayout(frameSalaires);

    QLabel *titreSalaires = new QLabel("💰 Statistiques des salaires");
    titreSalaires->setStyleSheet("font-size: 18px; font-weight: bold; color: #1e293b; margin-bottom: 15px;");

    QHBoxLayout *salairesLayout = new QHBoxLayout;
    salairesLayout->setSpacing(20);
    ...
    QString salaryStyle = "font-size: 15px; font-weight: bold; color: #047857; padding: 12px; background: #f1f5f9; border-radius: 10px; border-left: 5px solid #10b981;";
    ...
    lineRecherche->setMinimumHeight(45);
    lineRecherche->setStyleSheet("padding: 12px; border: 1px solid #cbd5e1; border-radius: 10px; background: white; font-size: 14px;");
    ...
    tableEmployes->setMinimumHeight(400);
    tableEmployes->setAlternatingRowColors(true);
    tableEmployes->setStyleSheet("background: white; border: 1px solid #e2e8f0; border-radius: 10px;");
...
void GestionUtilisateurs::actualiserTable()
{
    ...
    for (const Employe &e : listeEmployes) {
        ...
        btnEdit->setMaximumWidth(45);
        btnEdit->setMinimumHeight(35);
        btnEdit->setStyleSheet("background-color: #3b82f6; color: white; border-radius: 6px; font-size: 16px; padding: 5px;");
        btnEdit->setCursor(Qt::PointingHandCursor);

        QPushButton *btnDelete = new QPushButton("🗑️");
        btnDelete->setToolTip("Supprimer");
        btnDelete->setMaximumWidth(45);
        btnDelete->setMinimumHeight(35);
        btnDelete->setStyleSheet("background-color: #ef4444; color: white; border-radius: 6px; font-size: 16px; padding: 5px;");
        ...
        tableEmployes->setRowHeight(row, 55);
