...
void DialogNavire::configurerInterface()
{
    ...
    titre->setStyleSheet("font-size: 20px; font-weight: bold; color: #1e3a8a;");
    ...
    btnEnregistrer->setMinimumHeight(40);
    ...
    if (modeAdmin && modeModification) {
        ...
        btnSupprimer->setMinimumHeight(40);
...
void GestionNavires::configurerTableauBord()
{
    ...
    tableNavires->setMinimumHeight(400);
    tableNavires->setStyleSheet("background: white; border: 1px solid #e2e8f0; border-radius: 8px;");
...
void GestionNavires::configurerGestion()
{
    ...
    formFrame->setStyleSheet("background-color: white; border-radius: 12px; border: 1px solid #e2e8f0; padding: 20px;");
    ...
    titreForm->setStyleSheet("font-size: 18px; font-weight: bold; color: #1e3a8a;");
    titreForm->setAlignment(Qt::AlignCenter);

    champNom = new QLineEdit;
    champNom->setPlaceholderText("Ex: Neptune");
    champNom->setStyleSheet("padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: white;");

    champImmat = new QLineEdit;
    champImmat->setPlaceholderText("Ex: 123456789");
    champImmat->setStyleSheet("padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: white;");

    comboCapacite = new QComboBox;
    comboCapacite->addItems({"50","100","150","200","250","291","300","350","400"});
    comboCapacite->setStyleSheet("padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: white;");

    comboStatut = new QComboBox;
    comboStatut->addItems({"À quai","En mer","Interdit"});
    comboStatut->setStyleSheet("padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: white;");

    QPushButton *btnAjouterNavire = new QPushButton("➕ Ajouter le navire");
    btnAjouterNavire->setStyleSheet("background-color: #3b82f6; color: white; padding: 14px; border-radius: 8px; font-weight: bold; font-size: 14px;");
    ...
    titreTable->setStyleSheet("font-size: 16px; font-weight: bold; color: #1e293b;");
    ...
    tableGestion->setMinimumHeight(400);
    tableGestion->setStyleSheet("background: white; border: 1px solid #e2e8f0; border-radius: 8px;");
...
QFrame* GestionNavires::creerCarteStatistique(QString titre, QString valeur, QString icone, QString couleur)
{
    ...
    carte->setStyleSheet("QFrame { background-color: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; }");
    ...
    valLabel->setStyleSheet(QString("font-size: 36px; font-weight: bold; color: %1;").arg(couleur));
    valLabel->setAlignment(Qt::AlignCenter);

    QLabel *titreLabel = new QLabel(titre);
    titreLabel->setStyleSheet("font-size: 14px; color: #64748b; font-weight: 600;");
...
void GestionNavires::chargerNavires()
{
    ...
    for (const Navire &n : listeNavires) {
        ...
        tableNavires->setRowHeight(row, 50);
    }
    ...
    if (tableGestion) {
        ...
        for (const Navire &n : listeNavires) {
            ...
            tableGestion->setRowHeight(row, 45);
        }
    }
}
